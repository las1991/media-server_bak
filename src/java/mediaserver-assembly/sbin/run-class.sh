...
if [ -z "$JMEDIA_NETTY_OPTS" ]; then
  JMEDIA_NETTY_OPTS=" -Dio.netty.allocator.type=pooled -Dio.netty.noPreferDirect=true  -Dio.netty.leakDetection.level=DISABLED"
fi
...
if [ "x$DAEMON_MODE" = "xtrue" ]; then
  nohup $JAVA $JMEDIA_OPTS $JMEDIA_HEAP_OPTS$JMEDIA_JVM_PERFORMANCE_OPTS $JMEDIA_NETTY_OPTS  $JMEDIA_GC_LOG_OPTS $JMEDIA_JMX_OPTS $JMEDIA_LOG4J_OPTS -cp $CLASSPATH   "$@" >> "$CONSOLE_OUTPUT_FILE" 2>&1 < /dev/null &
else
  exec $JAVA $JMEDIA_OPTS $JMEDIA_HEAP_OPTS $JMEDIA_JVM_PERFORMANCE_OPTS $JMEDIA_NETTY_OPTS  $JMEDIA_GC_LOG_OPTS $JMEDIA_JMX_OPTS $JMEDIA_LOG4J_OPTS  -cp $CLASSPATH   "$@"
fi
