...
public class JitterBuffer implements Serializable {
    ...
    private final static Logger logger = LoggerFactory.getLogger(JitterBuffer.class);
    /**
    ...
    private void initJitter(RtpPacket firstPacket) {
        ...
        long firstPacketTimestamp = firstPacket.getTime();
	...
	private void estimateJitter(RtpPacket newPacket) {
		long arrival = rtpClock.getLocalRtpTime();
		long newPacketTimestamp = newPacket.getTime();
	...
	public void write(RtpPacket packet, RTPFormat format) {
		...
		if (isn == -1) {
			rtpClock.synchronize(packet.getTime());
		...
		if (packet.getTime() < this.arrivalDeadLine) {
			logger.warn("drop packet: dead line=" + arrivalDeadLine + ", packet time=" + packet.getTime() + ", seq=" + packet.getSeqNumber() + ", payload length=" + packet.getPayloadLength() + ", format=" + this.format.toString());
			dropCount++;
		...
		f.setTimestamp(rtpClock.convertToAbsoluteTime(packet.getTime()));
		f.setOffset(0);
