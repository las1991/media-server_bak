...
import com.sengled.media.server.rtsp.rtcp.handler.InterleavedRtcpHandler;
...
public class AnnounceStreamServlet extends RtspServletAdapter {
    ...
    public void record(FullHttpRequest request, FullHttpResponse response) {
        ...
        if (null != mediaSink) {
            ...
            getCtx().pipeline().addAfter(EXECUTOR_GROUP, "rtcpDecoder", "rtcpHandler", new InterleavedRtcpHandler(mediaSink));
            mediaSink.start();
            recorded = true;
        } else {
            response.setStatus(HttpResponseStatus.BAD_REQUEST);
